                     ...
                     {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
                         ...
                         <td class="fieldValue" {if $FIELD_MODEL->getFieldDataType() eq 'boolean'} style="width:25%" {/if} {if $FIELD_MODEL->get('uitype') eq '19'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
                            ...
                            {if $RECORD_ID neq ''}
                                ...
                                {if $FIELD_MODEL->getFieldName() eq 'subject'}
                                    {if $MODULE eq 'Invoice'}
                                       <input id="Invoice_editView_fieldName_subject" type="hidden" data-fieldname="subject" data-fieldtype="string" class="inputElement nameField" name="subject" value="{$RECORD_STRUCTURE_MODEL->getRecordName()}" data-rule-required="true" aria-required="true">
                                    {/if}
                                    {if $MODULE eq 'Quotes'}
                                         <input id="Quotes_editView_fieldName_subject" type="hidden" data-fieldname="subject" data-fieldtype="string" class="inputElement nameField" name="subject" value="{$RECORD_STRUCTURE_MODEL->getRecordName()}" data-rule-required="true" aria-required="true">
                                    {/if}
                                    {if $MODULE eq 'PurchaseOrder'}
                                         <input id="PurchaseOrder_editView_fieldName_subject" type="hidden" data-fieldname="subject" data-fieldtype="string" class="inputElement nameField" name="subject" value="{$RECORD_STRUCTURE_MODEL->getRecordName()}" data-rule-required="true" aria-required="true">
                                    {/if}
                                    {if $MODULE eq 'SalesOrder'}
                                         <input id="SalesOrder_editView_fieldName_subject" type="hidden" data-fieldname="subject" data-fieldtype="string" class="inputElement nameField" name="subject" value="{$RECORD_STRUCTURE_MODEL->getRecordName()}" data-rule-required="true" aria-required="true">
                                    {/if}
                                {/if}

                                {if $FIELD_MODEL->getFieldName() eq 'subject' and  ($MODULE eq 'PurchaseOrder' or  $MODULE eq 'Quotes' or  $MODULE eq 'Invoice' or  $MODULE eq 'SalesOrder')}
                                   <input type="text" value="{$RECORD_STRUCTURE_MODEL->getRecordName()}" class="inputElement nameField" disabled>
                                {else}
                            ...
                            {else}
                                {if $FIELD_MODEL->getFieldName() eq 'subject'}
                                    {if $MODULE eq 'Invoice'}
                                       <input id="Invoice_editView_fieldName_subject" type="hidden" data-fieldname="subject" data-fieldtype="string" class="inputElement nameField" name="subject" value="{{$CURRENTDATE_CUSTOM_TITLE}}" data-rule-required="true" aria-required="true">
                                    {/if}
                                     {if $MODULE eq 'Quotes'}
                                         <input id="Quotes_editView_fieldName_subject" type="hidden" data-fieldname="subject" data-fieldtype="string" class="inputElement nameField" name="subject" value="{{$CURRENTDATE_CUSTOM_TITLE}}" data-rule-required="true" aria-required="true">
                                    {/if}
                                    {if $MODULE eq 'PurchaseOrder'}
                                         <input id="PurchaseOrder_editView_fieldName_subject" type="hidden" data-fieldname="subject" data-fieldtype="string" class="inputElement nameField" name="subject" value="{{$CURRENTDATE_CUSTOM_TITLE}}" data-rule-required="true" aria-required="true">
                                    {/if}
                                     {if $MODULE eq 'SalesOrder'}
                                         <input id="SalesOrder_editView_fieldName_subject" type="hidden" data-fieldname="subject" data-fieldtype="string" class="inputElement nameField" name="subject" value="{{$CURRENTDATE_CUSTOM_TITLE}}" data-rule-required="true" aria-required="true">
                                    {/if}
                                {/if}
                                {if $FIELD_MODEL->getFieldName() eq 'subject' and  ($MODULE eq 'PurchaseOrder' or  $MODULE eq 'Quotes' or  $MODULE eq 'Invoice'or  $MODULE eq 'SalesOrder')}
                                   <input type="text" value="{{$CURRENTDATE_CUSTOM_TITLE}}" class="inputElement nameField" disabled>
                                {else}
                                    {include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE)}   
                                 {/if}
                            {/if}   
