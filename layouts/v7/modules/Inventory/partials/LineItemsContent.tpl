...
{strip}
	...
	{if $PRODUCT_EDITABLE}
		<td {if {$data.$productName} eq 'Product_Name'} class="product-prod" {else} class="prod-service" {/if}>
    ...
    {if $MARGIN_EDITABLE && $PURCHASE_COST_EDITABLE}
        <td>
            ...
            <select name="{$margin}" id="service_validty" class="form-control">
                {foreach key=val item=validity from=getLangArr('', 'Validity')}
                    <option {if $data.$margin == $val}selected{/if} value="{strtolower($val)}">{vtranslate($validity, 'Validity')}</option>
                {/foreach}
            </select>
            {*<textarea name="{$margin}" class="form-control">{vtranslate('validity')}</textarea>*}
	...
	{if $PURCHASE_COST_EDITABLE or $MODULE eq 'SalesOrder'}
		<td>
			{*<input id="{$purchaseCost}" type="hidden" value="{if $data.$purchaseCost}{((float)$data.$purchaseCost) / ((float)$data.$qty)}{else}0{/if}" />*}
			<input name="{$purchaseCost}" type="text" value="{if $data.$purchaseCost}{$data.$purchaseCost}{else}0{/if}" />
	...
	<td>
        <input type="hidden" name="netprice" value="{if $data.$netPrice}{$data.$netPrice}{else}0{/if}">
