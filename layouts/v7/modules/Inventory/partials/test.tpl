...
{/if}
    ...
    {if $PRODUCT_EDITABLE}
        <td {if {$data.$hdnProductId} eq 600} colspan="5" style="overflow: hidden" {/if}>
                ...
                <div class="row">
                    <div class="col-lg-10">
                        <div class="input-group" style="width:100%">
                            {if {$data.$hdnProductId} neq 600}
                                <style>.vicon-products { display: none; }</style>
            ...
            {else}
                {if {$data.$hdnProductId} eq 600}
                    {if $COMMENT_EDITABLE}
                        <div><textarea style="width: 100%; text-transform: uppercase; font-family: 'OpenSans-Semibold', 'ProximaNova-Semibold', sans-serif;
    font-weight: normal;
    font-size: 2em;" id="{$comment}" name="{$comment}" class="lineItemCommentBox">{decode_html($data.$comment)}</textarea></div>
                    {/if}
                {/if}
            {/if}
        </td>
