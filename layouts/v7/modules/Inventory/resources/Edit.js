...
if(!GetURLParameter('record')) {
	// $('#row1').hide();
	$('#row1').find('.input-group').hide();
}

// if(GetURLParameter('record') === undefined) {
// 	$('#lineItemTab #row1').hide();
// }

	var prod = $('.product-prod');
	var serv = $('.prod-service');

	if (prod.length > 0) {
		prod.each(function() {
			// $(this).parent().find('.input-group input:not([type="hidden"]), .input-group span, .input-group div').remove();
			$(this).parent().find('td:not(:nth-child(1)):not(:nth-child(2))').html('');
			$(this).parent().find('td').eq(1).attr('colspan', '6').css('overflow', 'hidden');
			$(this).parent().find('td textarea').css({'width': '35%', 'font-size': '1.1em'});
			$(this).parent().find('td:empty').remove();
		})

	}
	if (serv.length > 0) {
		serv.each(function () {
			$(this).parent().find('.lineItemCommentBox').hide();
		});
	}
...
//		this.calculateGrandTotal();
    ...
    registerAddProductService : function() {
        ...
        var addLineItemEventHandler = function(e, data){
            ...
            if(type == 'addProduct') {
				newLineItem.find('.input-group input:not([type="hidden"]), .input-group span, .input-group div').remove();
				newLineItem.find('td:not(:nth-child(1)):not(:nth-child(2))').html('');
				newLineItem.find('td').eq(1).attr('colspan', '6').css('overflow', 'hidden');
				newLineItem.find('td textarea').css({'width' : '35%', 'font-size' : '1.1em'});
				// newLineItem.find('td textarea').after('<button class="btn btn-danger btn-sm" type="button" onclick="$(this).prev().remove();$(this).remove();" style="margin-top: -35px;margin-left: 10px;">Delete</button>');
				newLineItem.find('td:empty').remove();
				newLineItem.find('input[id^="hdnProductId"]').val(600);
				newLineItem.find('.lineItemCommentBox').show();
			} else if(type == 'addService') {

				//newLineItem.find('td textarea').remove();
			}
        }
