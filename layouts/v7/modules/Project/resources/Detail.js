...
// if(location.href.indexOf('tab_label=Quote') > -1) {
// 	// $('button[data-modulename="Quotes"]').remove();
// 	if(GetURLParameter('record') !== undefined) {
// 		$('.col-lg-6.col-md-6.col-sm-6.btn-toolbar .btn-group').append('<a href="/index.php?module=Quotes&view=Edit&app=SALES&relProject=' + GetURLParameter('record') + '" class="btn btn-default selectRelation">Create Quote</a>')
// 	}
// }
...
Vtiger_Detail_Js("Project_Detail_Js",{
	...
	*/
    ...
    loadContents : function(url,data) {
		// $('.col-lg-6.col-md-6.col-sm-6.btn-toolbar .btn-group').append('<a href="/index.php?module=Quotes&view=Edit&app=SALES" style="margin-left: 20px" class="btn btn-default selectRelation">Create Quote</a>')
	...
	registerEventsForTasksWidget : function(summaryViewContainer) {
		...
		tasksWidget.on('click', '.checkTask', function(e) {
			var currentTarget = jQuery(e.currentTarget);
			var value = currentTarget.value !== undefined ? currentTarget.value : currentTarget.val();
				var params = {
					action : 'SaveAjax',
					record : currentTarget.data('recordid'),
					field : currentTarget.data('fieldname'),
					value : value == app.vtranslate('Completed') ? app.vtranslate('Open') : app.vtranslate('Completed'),
					module : 'ProjectTask'
				};
				app.helper.showProgress();
				app.request.post({data: params}).then(
