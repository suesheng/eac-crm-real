...
if(GetURLParameter('relProject') !== undefined) {
	$('input[name="cf_905"]').val(GetURLParameter('relProject'));
	$('.editViewContents').append("<input type='hidden' name='relProject' value='"+ GetURLParameter('relProject') +"'>");
}

let format = $('#Quotes_editView_fieldName_validtill').data('date-format').toUpperCase();
var incMonth = moment(moment()).add(30, 'days').format(format);

if($('#Quotes_editView_fieldName_validtill').val().length <= 0) {
	$('#Quotes_editView_fieldName_validtill').val(incMonth);
}
