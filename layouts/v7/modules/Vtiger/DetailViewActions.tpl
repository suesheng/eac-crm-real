        ...
        <div class="pull-right btn-toolbar">
            ...
            {foreach item=DETAIL_VIEW_BASIC_LINK from=$DETAILVIEW_LINKS['DETAILVIEWBASIC']}
                <button class="btn btn-default" id="{$MODULE_NAME}_detailView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_BASIC_LINK->getLabel())}"
                        {if $DETAIL_VIEW_BASIC_LINK->isPageLoadLink()}
                            onclick="window.location.href = '{$DETAIL_VIEW_BASIC_LINK->getUrl()}&app={$SELECTED_MENU_CATEGORY}&relProject={$smarty.get.relProject}'"
                        {else}
            ...
            {/foreach}
                {foreach item=DETAIL_VIEW_LINK from=$DETAILVIEW_LINKS['DETAILVIEW']}
                        ...
                        {if {Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_LINK->getLabel())} == 'Send_Email_with_PDF' }{continue}{/if}
                        {if {Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_LINK->getLabel())} == 'Export_to_PDF' }{continue}{/if}
                        {*<li id="{$MODULE_NAME}_detailView_moreAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_LINK->getLabel())}">*}
                            {if $DETAIL_VIEW_LINK->getUrl()|strstr:"javascript"}
                                <a class="btn btn-default" href='{$DETAIL_VIEW_LINK->getUrl()}'>{vtranslate($DETAIL_VIEW_LINK->getLabel(), $MODULE_NAME)}</a>
                            {else}
                                <a class="btn btn-default" href='{$DETAIL_VIEW_LINK->getUrl()}&app={$SELECTED_MENU_CATEGORY}&relProject={$smarty.get.relProject}' >{vtranslate($DETAIL_VIEW_LINK->getLabel(), $MODULE_NAME)}</a>
                            {/if}
                        {*</li>*}
                ...
                {*<ul class="dropdown-menu dropdown-menu-right">*}
                    {*{foreach item=DETAIL_VIEW_LINK from=$DETAILVIEW_LINKS['DETAILVIEW']}*}
                        {*{if $DETAIL_VIEW_LINK->getLabel() eq ""} *}
                            ...
                            {*<li id="{$MODULE_NAME}_detailView_moreAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_LINK->getLabel())}">*}
                                {*{if $DETAIL_VIEW_LINK->getUrl()|strstr:"javascript"} *}
                                    {*<a href='{$DETAIL_VIEW_LINK->getUrl()}&relProject={$smarty.get.relProject}'>{vtranslate($DETAIL_VIEW_LINK->getLabel(), $MODULE_NAME)}</a>*}
                                {*{else}*}
                                    {*<a href='{$DETAIL_VIEW_LINK->getUrl()}&app={$SELECTED_MENU_CATEGORY}&relProject={$smarty.get.relProject}' >{vtranslate($DETAIL_VIEW_LINK->getLabel(), $MODULE_NAME)}</a>*}
                                {*{/if}*}
                            {*</li>*}
                        {*{/if}*}
                    {*{/foreach}*}
