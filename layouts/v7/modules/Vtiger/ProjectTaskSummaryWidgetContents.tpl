				...
				<li>
					<div>
						<div class="textOverflowEllipsis width27em">
							...
							<input type="checkbox" data-recordid="{$RELATED_RECORD->getId()}" data-fieldname="projecttaskstatus"
								    id="rec-{$RELATED_RECORD->getId()}" class="checkTask" {if $RELATED_RECORD->getDisplayValue('projecttaskstatus') eq vtranslate('Completed', $RELATED_MODULE)}checked{/if} value="{$RELATED_RECORD->getDisplayValue('projecttaskstatus')}"> &nbsp;
							...
							<a href="{$RELATED_RECORD->getDetailViewUrl()}" id="{$MODULE}_{$RELATED_MODULE}_Related_Record_{$RELATED_RECORD->get('id')}" title="{$RELATED_RECORD->getDisplayValue('projecttaskname')}">
								{if $RELATED_RECORD->getDisplayValue('projecttaskstatus') eq vtranslate('Completed', $RELATED_MODULE)}<del>{/if}<strong>{$RELATED_RECORD->getDisplayValue('projecttaskname')}</strong>{if $RELATED_RECORD->getDisplayValue('projecttaskstatus') eq 'Completed'}</del>{/if}
							</a>
						</div>
						<div class="row">
