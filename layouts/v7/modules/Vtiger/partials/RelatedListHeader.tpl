...
{strip}
					...
					<div class="btn-group">
						...
						{else}
							...
							{if $IS_SELECT_BUTTON || $IS_CREATE_PERMITTED}
								{if $RELATED_MODULE_NAME=="Contacts"}
									<a class="btn btn-default" href="{$RELATED_LINK->getUrl()}{if $SELECTED_MENU_CATEGORY}&app={$SELECTED_MENU_CATEGORY}{/if}"><i class="fa fa-plus"></i> {$RELATED_LINK->getLabel()}</a>
									{else}
  ...
  function getLink() {
  	return '/index.php?module=Quotes&view=Edit&app=SALES&relProject=' + GetURLParameter('record');
  }
							</script>
							<a onclick="location.href = getLink();" class="btn btn-default selectRelation">{vtranslate('Create Quote', 'Quotes')}</a>
