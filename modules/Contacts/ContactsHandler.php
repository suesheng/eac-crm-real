...
function Contacts_sendCustomerPortalLoginDetails($entityData){
	...
	$isPortalChanged = $entityDelta->hasChanged($moduleName, $entityId, 'portal') && $isPortalEnabled;//changed and not empty
...
//		$sqlC = "SELECT customer_id, portal FROM `vtiger_customerdetails` WHERE customer_id = ?";
//
//		$resultC = $adb->pquery($sqlC, array($entityId));
		...
		if ($update || $isEmailChanged || $isPortalChanged) {
