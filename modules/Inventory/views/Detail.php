...
//		var_dump($relatedProducts); exit;
		...
		for ($i=1; $i<=$productsCount; $i++) {
			...
			$pr_id = $product['hdnProductId'.$i];
			$pr_price = $product["listPrice".$i];
			//$servicesArr = ($product['hdnProductId'.$i] != 217)? $ordering++ : $ordering ;
			if($pr_id != 217  ){
				$servicesArr[$pr_id][$pr_price]["qty"] = $servicesArr[$pr_id][$pr_price]["qty"]+1 ;
				if( $servicesArr[$pr_id][$pr_price]['ord'] == NULL ){
					$servicesArr[$pr_id][$pr_price]['ord'] = $ordering;
					$ordering++;
				}
			}
//var_dump($product['hdnProductId'.$i]);exit;
			...
			$currencyFieldsList = array('taxTotal', 'netPrice', 'listPrice', 'unitPrice', 'productTotal','purchaseCost',
		...
		$viewer->assign('RECORD', $recordModel);
		$viewer->assign('MODULE_NAME',$moduleName);
		$viewer->assign('SERVICE_ARRAY',$servicesArr);
