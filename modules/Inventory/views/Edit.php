...
Class Inventory_Edit_View extends Vtiger_Edit_View {
	...
	//	$last_record =getInstanceById();
	//var_dump($moduleName);
		$date_curent = date('Y-n-j');

		global $adb;
		$all_record_thid_day = "SELECT count(crmid) as count_date FROM vtiger_crmentity WHERE setype = '".$moduleName."' AND createdtime >= DATE('".$date_curent."')";
		if($moduleName == 'Invoice'){
		
				$title1 = "RE-";
				$result_time_record = $adb->pquery($all_record_thid_day);
				$result_info_time = (array) $result_time_record;
			
			$no_doc = $result_info_time['fields']['count_date']+1;
			if($no_doc < 10){$no_doc_final = "000".$no_doc; }elseif($no_doc >= 10){$no_doc_final = "00".$no_doc; }elseif($no_doc >= 99){$no_doc_final = "0".$no_doc; }
			$title_doc = $title1.$date_curent."/".$no_doc_final;
		...
		$viewer->assign('CURRENTDATE_CUSTOM_TITLE', $title_doc);
		...
		$result_last_record = $adb->pquery($info_last_record);
		$result_info = (array) $result_last_record;

		//var_dump($result_info['fields']['subject']);	
		//var_dump($title_doc);
